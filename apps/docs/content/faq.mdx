...
Not directly. `ComputerManager.ensure(key, spec)` maps a stable key to a machine, and
the agent's key is `husk:<name>`. Two husks are two keys and therefore two machines. The
control plane accepts a `computerId` in a run body and
[never reads it](/reference/api#running-a-husk).
