...
`fetch`, takes its shared types from `@husk/core`, and speaks exactly the routes
...
boots a **real** `@husk/server` on an ephemeral port, backs it with a **real**
...
- **`fs/upload` and `fs/download`.** No route serves them: the server registers
  no multipart parser and ships no tar implementation. A method that exists only
  to throw still autocompletes and still reads as supported. Use `writeFile` and
  `readFile`.
